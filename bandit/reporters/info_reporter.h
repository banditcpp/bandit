...
namespace bandit {
  namespace detail {
    struct info_reporter : public colored_reporter {
      struct context_info {
        context_info(const std::string& d)
            : desc(d), total(0), skipped(0), failed(0) {
        }
      ...
      info_reporter(std::ostream& stm, const failure_formatter& failure_formatter, const colorizer& colorizer)
          : colored_reporter(stm, failure_formatter, colorizer), indentation_(0), not_yet_shown_(0), context_stack_() {
      }
